...
This gate makes no editorial or visual selection. It only checks that every authored
Financial Visual Template has an explicit authored financial binding and that those
bindings point back to the exact Scene/Beat/template they claim to drive. The set of
Financial Visual Templates is derived from the existing financial recipe registry so
this script does not introduce a second template authority.
"""
...
def financial_template_ids(registry: dict[str, Any]) -> set[str]:
    ...
    if not result:
        raise AuthoringClosureError("financial recipe registry exposes no preferred Financial Visual Templates")
    return result
...
def validate_authoring(authoring: dict[str, Any], registry: dict[str, Any]) -> list[str]:
    ...
    beat_map: dict[str, tuple[int, int, dict[str, Any]]] = {}
    for scene_index, scene in enumerate(scenes, 1):
        ...
        for beat_index, beat in enumerate(beats, 1):
            ...
            if not isinstance(beat, dict):
                ...
                continue
            beat_map[beat_id] = (scene_index, beat_index, beat)
    ...
    for beat_id, (_, _, beat) in beat_map.items():
        template = beat.get("visualTemplate")
        if template not in financial_templates:
            continue
        binding = by_beat.get(beat_id)
        if binding is None:
            errors.append(
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--registry",
        type=Path,
        default=Path("contracts/financial_recipe_registry.json"),
    )
