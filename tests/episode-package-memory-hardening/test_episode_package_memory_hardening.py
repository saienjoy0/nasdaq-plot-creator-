...
class HardeningTests(unittest.TestCase):
    ...
    def test_03_all_nine_scenes_required(self):
        self.package.write_text(
            build_package(scenes=list(range(1, 9))), encoding="utf-8"
        )
        self.assertFail("Scene 1 through Scene 9")

    def test_04_duplicate_scene_rejected(self):
        self.package.write_text(
            build_package(scenes=[1, 2, 3, 4, 5, 5, 6, 7, 8, 9]),
            encoding="utf-8",
        )
    ...
    def test_05_out_of_order_scene_rejected(self):
        self.package.write_text(
            build_package(scenes=[1, 2, 4, 3, 5, 6, 7, 8, 9]),
            encoding="utf-8",
        )
        self.assertFail("exactly once and in order")
    ...
    def test_06_inquisition_required(self):
        self.package.write_text(
            build_package(include_inquisition=False), encoding="utf-8"
        )
    ...
    def test_10_public_spoken_script_must_not_leak_marker(self):
        ...
        spoken.write_text(
            "hello <!--MEMREF:MR-001:U-001-->", encoding="utf-8"
        )
        self.assertFail(
            "internal episode-memory metadata", public_artifacts=[spoken]
        )

    def test_11_public_render_spec_must_not_leak_annex(self):
        render = self.root / "render_spec.json"
        render.write_text(
            '{"text":"<!--BEGIN_EPISODE_MEMORY_ANNEX-->"}', encoding="utf-8"
        )
        self.assertFail(
            "internal episode-memory metadata", public_artifacts=[render]
        )
    ...
    def test_13_clean_public_artifacts_pass(self):
        ...
        self.assertEqual(
            [], self.validate(public_artifacts=[spoken, render]).errors
        )
    ...
    def test_14_public_artifact_outside_repo_rejected(self):
        ...
        try:
            self.assertFail(
                "escapes repository root", public_artifacts=[outside]
            )
        finally:
